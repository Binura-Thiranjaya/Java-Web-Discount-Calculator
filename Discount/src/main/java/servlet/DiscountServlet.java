	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!customerId.equals("Select ID")) {
			try {
				...
				out.println("<h1 style='text-align: right;'>Customer ID:<span style='color:red;'>"+customerId+"</span></h1>");
				...
				while (rs.next()) {
					...
					out.print("<tr> "
							+ "<td>" + Integer.parseInt(rs.getString("inv_number")) + "</td>"
							+"<td> "+ Integer.parseInt(rs.getString("customer_code")) +"</td>"
							+"<td> "+ rs.getString("customer_name") +"</td>"
							+"<td> <b>Rs. "+ Discount +" .00</b></td>"
							+ "</tr>");	
				}
				out.println("</table>");
